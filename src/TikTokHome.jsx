...
// +1 发送图标 — Figma node 143-3119 (白色填充，黑色描边 0.75 透明度)
...
// isPlusOne: 是可以+1的弹幕; hasSentPlusOne: 用户已点击+1
function DanmakuItem({ text, isUser, isPlusOne, hasSentPlusOne, myKey, activeKey, likeCount, baseCount, onItemClick, onEnd, onMeasure }) {
  ...
  // 样式决策
  const showAsUser = isUser || (isPlusOne && hasSentPlusOne);   // 已发送样式
  const showPlusOne = isPlusOne && !hasSentPlusOne;             // 待点击的+1样式
...
export default function TikTokHome({ className, videoSrc, username, description, avatarSrc, captionOffset = 0, presetDanmakus = [], bgTexts }) {
  ...
  const [userDanmakus, setUserDanmakus] = useState(
    presetDanmakus.map(d => ({ id: crypto.randomUUID(), text: d.text, row: d.row ?? 0, isPreset: true, isPlusOne: d.plusOne ?? false }))
  );
  ...
  function handlePlusOne(key, text, rowIndex) {
    setPlusOneSent(prev => new Set([...prev, key]));
    const id = crypto.randomUUID();
    setUserDanmakus(prev => [...prev, { id, text, row: rowIndex }]);
  }

  function handleDanmakuClick(e, key, rowIndex, text, isPlusOne) {
    e.stopPropagation();
    if (isPlusOne) { handlePlusOne(key, text, rowIndex); return; }
